...
int main(int argc, char **argv) {
	...
	*/
	device = "/dev/i2c-1";
	int l_timer=30000000; //Как часто опрашиваем датчики и бегаем по циклу. По дефолту 30 секунд
	int l_count_current=0; //Текущий счетчик
	int l_count_for_commit_to_base=0;//На какой итерации мы коммитим в базу. 1=каждый раз
	int l_count_for_send_narodmon=0;//На какой итерации мы сливаем на сайт(заготовка). 1=каждый раз
	
	connection C("dbname=smarthome user=postgres password=odroid \
	...
	while (1) {
		...
		printf("IR : %.0f Lux\n", Si1132_readIR());
			...
			bme280_read_pressure_temperature_humidity(
			    &pressure_hpa, &temperature, &humidity);
			printf("======== bme280 ========\n");
			...
			printf("altitude : %f m\n", bme280_readAltitude(pressure_hpa,
...
try {
	...
	work W(C);	
				...
				W.prepared("insert_item")(3)(pressure_hpa/100.0).exec(); //bme280 pressure_hpa
				W.prepared("insert_item")(4)(bme280_readAltitude(pressure_hpa,
			        SEALEVELPRESSURE_HPA)).exec(); //bme280 altitude
